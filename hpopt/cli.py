...
@app.command()
def tune(n_trials: int = 10, study_name: str = "distributed-amptorch-tuning", with_db: bool = False, pruner: str = "Median", sampler: str = "CmaEs", verbose: bool = False, n_epochs: int = 100):
    ...
    local = "on DB" if with_db else "locally"
    study = get_or_create_study(study_name=study_name, with_db=with_db, pruner=pruner, sampler=sampler)
    objective = mk_objective(verbose=verbose, epochs=n_epochs)
    study.optimize(objective, n_trials=n_trials)

@app.command()
def create_lmdbs(train: str = "train.traj", valid: str = "valid.traj", test: str = "test.traj") -> None:
...
@app.command()
def train_valid_split(train: str = "oc20_3k_train.traj", valid_split: float = 0.1, train_out_fname: str = "train.traj", valid_out_fname: str = "valid.traj") -> None:
    ...
    create_validation_split(train_fname=train, valid_split=valid_split, train_out_fname=train_out_fname, valid_out_fname=valid_out_fname)
...
@app.command()
def run_tuning_jobs(n_jobs: int = 5, n_trials_per_job: int = 5, study_name: str = "distributed-amptorch-tuning", pruner="Median", sampler="CmaEs"):
    from hpopt.jobs import run_tuning_jobs

    run_tuning_jobs(n_jobs=n_jobs, n_trials_per_job=n_trials_per_job, study_name=study_name, pruner=pruner, sampler=sampler)
