...
def get_or_start_mysql_job():
    qstat_out = run("qstat -u $USER -n1", shell=True, capture_output=True).stdout.decode('utf-8')
    
    jobs = pd.DataFrame(
      [r.split() for r in qstat_out.splitlines() if r and r[0].isnumeric()],
      columns=['job_id', 'username', 'queue', 'jobname', 'sessid', 'nds', 'tsk', 'memory', 'time', 'status', 'elapsed', 'node'],
    )
